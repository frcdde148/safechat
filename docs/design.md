...
- TGS 数据库：服务配置、TGS 审计。
- ChatServer 数据库：用户角色副本、聊天记录、图片数据或图片密文、离线消息、禁言规则、会话撤销、聊天审计。
...
- 文本消息使用 `Kc,v` 加密传输。
- 图片正文固定使用 AES-GCM 和 `Kc,v` 加密图片 bytes。
- 聊天历史保存明文文本、服务端 AES-GCM 图片密文、HMAC、签名和发送者公钥，便于客户端展示安全层信息；返回给客户端时文本使用当前 `Kc,v` 加密。
...
- 历史图片默认只显示占位，点击时才拉取并解密，避免批量传输阻塞 UI。
...
- AS/TGS 管理接口使用 `Kc,tgs` 加密 `admin_token` 和业务字段，服务端不接受旧版明文字段。
...
- 图片正文固定使用 AES-GCM 加密，`performance.encrypt_images` 不再作为图片协议开关。
